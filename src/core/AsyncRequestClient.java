...
public class AsyncRequestClient {
     ...
     * 每个请求中轨迹点数量
     */
    private static int pointSize = 1500;
    ...
    public void start() {
        if (!isRunning) {
            ...
            taskScheduleThread.start();   ////////启动后，执行该线程的run()方法，调用doTask()方法
        }
    }
    ...
    public void submitTask(long requestId, String action, String parameters, String method) {
        ...
        // 任务存放到待执行队列
            try {
                waitingQueue.put(taskCallable);//将轨迹数据集放入阻塞队列中，等待后续取操作
     ...
     * 执行任务
     */
    private static void doTask() {  ////////////执行任务，最终将数据上传
        TaskCallable<String> taskCallable = null;
        while (isRunning) {
            try {
                taskCallable = waitingQueue.take();  //在阻塞队列中取线程
                ...
                if (concurrencyControl()) {
                    ...
                    // 若超并发，休眠1s再执行
                    System.err.println("超并发，休眠10s, waitingQueue size : " + waitingQueue.size());
                    Thread.sleep(10000);
                    executorService.submit(taskCallable);
                }
            } catch (InterruptedException e) {
   ...
   /**
    ...
    public static final class TaskRunable implements Runnable {
        ...
        public void run() {
            // 发送请求
           // String result = AnalysisTool.postFormReq(UrlDomain.YINGYAN_HTTP_URL+UrlDomain.ACTION_ADD_POINTS, parameters);
    ...
    public static final class TaskScheduleThread extends Thread {
        ...
        public void run() {//运行线程，执行数据上传到百度鹰眼任务
            doTask();
        }//////////线程启动成功执行run（）方法，调用doTask()方法。
    }
